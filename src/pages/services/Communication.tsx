...
import { BookOpen, Globe, Megaphone, Sparkles } from 'lucide-react';
...
export default function Communication() {
  ...
  const intro = t('services.communication_branding.intro');
  const formation = t('services.communication_branding.formation', { returnObjects: true }) as {
    title: string;
    description: string;
  };
  const cta = t('services.communication_branding.cta', { returnObjects: true }) as {
    title: string;
    title2: string;
    text: string;
    btn: string;
      ...
      <section className="mx-auto flex w-full max-w-7xl flex-col px-4 pb-20 pt-28 sm:px-6 lg:px-8 lg:pt-36">
        ...
        >
          <p className="mb-4 text-sm font-semibold uppercase tracking-[0.35em] text-gold/80">
            {t('services.communication_branding.eyebrow')}
          </p>
          <h1 className="text-4xl font-black uppercase leading-[0.95] tracking-tight text-white sm:text-5xl lg:text-6xl">
            <span className="block">{t('services.communication_branding.title')}</span>
                  ...
                  >
                    <div className="mb-5 flex items-center justify-between">
                      ...
                      <div className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 text-white/60 transition-colors group-hover:text-gold">
                        {serviceIndex % 2 === 0 ? <Megaphone size={16} /> : <Sparkles size={16} />}
                      </div>
                    </div>
