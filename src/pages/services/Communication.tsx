...
export default function Communication() {
  ...
  const sections = t('services.communication_branding.sections', { returnObjects: true }) as SectionGroup[] || [];
  const intro = t('services.communication_branding.intro', { defaultValue: '' });
  const formation = t('services.communication_branding.formation', { returnObjects: true }) as { title: string; description: string; } || { title: '', description: '' };
  const cta = t('services.communication_branding.cta', { returnObjects: true }) as { title: string; title2: string; text: string; btn: string; } || { title: '', title2: '', text: '', btn: '' };
    ...
    <div className="min-h-screen bg-[#0A0A0C] text-[#F3F4F6] selection:bg-gold selection:text-black antialiased overflow-x-hidden font-sans">
      ...
      <section className="relative mx-auto w-full max-w-7xl px-6 pt-32 pb-24 sm:px-8 lg:pt-44 lg:pb-36 flex flex-col justify-center">
        ...
        >
          ...
          >
            {t('services.communication_branding.eyebrow', { defaultValue: 'COMMUNICATION & BRANDING' })}
          </motion.p>
          ...
          >
            <span className="block text-white opacity-95">{t('services.communication_branding.title')}</span>
      ...
      <section id="explore" className="mx-auto w-full max-w-7xl px-6 py-16 sm:px-8 border-t border-white/[0.04]">
        <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <span className="text-xs font-semibold uppercase tracking-widest text-gold/80">{t('services.communication_branding.sections_label')}</span>
            <h2 className="mt-2 text-3xl font-bold uppercase tracking-tight text-white sm:text-4xl">{t('services.communication_branding.sections_title')}</h2>
          </div>
          ...
          >
            ...
            <div className="max-w-3xl">
              <span className="text-xs font-bold uppercase tracking-widest text-gold/80 flex items-center gap-2 mb-2">
                <span className="w-1.5 h-1.5 bg-gold rounded-full" /> {t('services.communication_branding.formation_label')}
              </span>
        ...
        >
          ...
          <div className="relative z-10 max-w-2xl mx-auto space-y-6">
            ...
            <h2 className="text-3xl font-extrabold uppercase tracking-tight text-white sm:text-4xl lg:text-5xl leading-none">
              {cta.title2 || 'Ready to elevate your brand?'}
            </h2>
            
            <p className="text-sm sm:text-base text-gray-400 font-light leading-7 max-w-xl mx-auto">
              {cta.text || 'Let\'s discuss your communication and branding strategy. Together we identify the adjustments that will make the difference. First consultation free.'}
            </p>
