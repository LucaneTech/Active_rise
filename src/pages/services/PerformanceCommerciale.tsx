...
export default function Marketing() {
  ...
  const sections = t('services.performance_commerciale.sections', { returnObjects: true }) as SectionGroup[] || [];
  const intro = t('services.performance_commerciale.intro', { defaultValue: '' });
  const formation = t('services.performance_commerciale.formation', { returnObjects: true }) as { title: string; description: string; } || { title: '', description: '' };
  const cta = t('services.performance_commerciale.cta', { returnObjects: true }) as { title: string; title2: string; text: string; btn: string; } || { title: '', title2: '', text: '', btn: '' };
    ...
    <div className="min-h-screen bg-[#0A0A0C] text-[#F3F4F6] selection:bg-gold selection:text-black antialiased overflow-x-hidden font-sans">
      ...
      <section className="relative mx-auto w-full max-w-7xl px-6 pt-32 pb-24 sm:px-8 lg:pt-44 lg:pb-36 flex flex-col justify-center">
        ...
        >
          ...
          >
            {t('services.performance_commerciale.eyebrow', { defaultValue: 'PERFORMANCE & GROWTH' })}
          </motion.p>
          ...
          >
            <span className="block text-white opacity-95">{t('services.performance_commerciale.title')}</span>
            <span className="mt-3 block bg-gradient-to-r from-gold via-[#F3C66F] to-yellow-500 bg-clip-text text-transparent">
              {t('services.performance_commerciale.title2')}
            </span>
          </motion.h1>
      ...
      {/* 2. MODULE DE SERVICES INTERACTIF (ASYNCHRONOUS TABS & ACCORDIONS ARCHITECTURE) */}
      <section id="explore" className="mx-auto w-full max-w-7xl px-6 py-16 sm:px-8 border-t border-white/[0.04]">
        <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <span className="text-xs font-semibold uppercase tracking-widest text-gold/80">{t('services.performance_commerciale.sections_label')}</span>
            <h2 className="mt-2 text-3xl font-bold uppercase tracking-tight text-white sm:text-4xl">{t('services.performance_commerciale.sections_title')}</h2>
          </div>
          ...
          >
            ...
            <div className="max-w-3xl">
              <span className="text-xs font-bold uppercase tracking-widest text-gold/80 flex items-center gap-2 mb-2">
                <span className="w-1.5 h-1.5 bg-gold rounded-full" /> {t('services.performance_commerciale.formation_label')}
              </span>
        ...
        >
          ...
          <div className="relative z-10 max-w-2xl mx-auto space-y-6">
            <span className="text-xs font-bold uppercase tracking-[0.35em] text-gold">
              {cta.title || 'DÉMARRER LA CONVERSION'}
            </span>
            
            <h2 className="text-3xl font-extrabold uppercase tracking-tight text-white sm:text-4xl lg:text-5xl leading-none">
              {cta.title2 || 'Prêt à scaler votre croissance ?'}
            </h2>
            
            <p className="text-sm sm:text-base text-gray-400 font-light leading-7 max-w-xl mx-auto">
              {cta.text || 'Architecturons ensemble des tunnels de conversion à haute efficacité et optimisons vos performances d’acquisition.'}
            </p>
            ...
            <div className="pt-6">
              ...
              >
                {cta.btn || 'Lancer le projet via WhatsApp'}
