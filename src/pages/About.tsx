...
import { Target, Lightbulb, Shield, Star, Globe, BarChart3, Briefcase, Compass, Layers, Eye, HeartHandshake, ShieldCheck } from 'lucide-react';
...
// Interface pour typer le composant si nécessaire
interface SectionProps {
  RevealOnScroll: React.ComponentType<{
...
const StorySection: React.FC<SectionProps> = ({ RevealOnScroll }) => {
  const { t } = useTranslation()
  ...
  return (
    <section className="relative min-h-screen w-full flex items-center justify-center bg-[#0B0F19] py-12 md:py-20 overflow-hidden">
        ...
        <div className="w-full flex flex-col sm:flex-row sm:items-center sm:justify-between pb-8 mb-8 border-b border-white/5 gap-4">
          ...
          <div className="flex items-center gap-6 text-xs font-medium tracking-widest text-white/60 uppercase">
            <RevealOnScroll delay={0.1}>
              <span className="hover:text-gold transition-colors duration-300 cursor-pointer">Vision</span>
            </RevealOnScroll>
            <RevealOnScroll delay={0.2}>
              <span className="hover:text-gold transition-colors duration-300 cursor-pointer">Approche</span>
            </RevealOnScroll>
          </div>
        </div>
        ...
        <div className="w-full mt-auto pt-8 border-t border-white/5 z-10">
          <RevealOnScroll direction="up" delay={0.4}>
            <div className="grid md:grid-cols-2 gap-8 items-start bg-white/[0.02] backdrop-blur-md border border-white/5 p-6 md:p-8 rounded-2xl">
              <div>
                <h4 className="text-xs font-bold tracking-wider text-gold uppercase mb-2">Notre Stratégie</h4>
      ...
      <img src="images/icon.png" alt="icon active_rise" className='w-35 md:w-40 lg:w-80 h-35 md:h-40 lg:h-80 absolute -bottom-0 -right-0 z-4  opacity-90' />
      </div>
    </section>
  )
}
...
const VisionMissionSection: React.FC<SectionProps> = ({ RevealOnScroll }) => {
  const { t } = useTranslation()
  ...
  return (
    <section className="relative w-full bg-deep text-white py-24 lg:py-36 overflow-hidden">
      ...
      <div className="w-full  py-16 lg:py-24 relative overflow-hidden select-none">
        {/* Subtil motif géométrique linéaire en fond de citation */}
...
export const ValuesTimelineSection: React.FC<SectionProps> = ({ RevealOnScroll }) => {
  const { t } = useTranslation()
...
const differentiators = [
  ...
  return (
    <div className="bg-beige overflow-hidden">
      ...
      {/* ─── STORY ─── */}
      <StorySection RevealOnScroll="left" />

      {/* ─── VISION & MISSION ─── */}
      
      <VisionMissionSection RevealOnScroll="left"/>
      ...
      {/* ─── VALUES ─── */}
      <ValuesTimelineSection RevealOnScroll="down"/>
